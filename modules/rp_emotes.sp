...
{
	name = "Roleplay - [MODULE]Emotes",
	author = "Franc1sco (Reedited By MbK)",
	description = "Systeme d'emotes fortnite",
	version = "1.0",
...
{	
	...
	LoadTranslations("rp_emotes.phrases");
	...
	RegConsoleCmd("rp_emotes", Command_Menu);	
	RegConsoleCmd("rp_emote", Command_Menu);
	RegConsoleCmd("rp_dance", Command_Menu);	
	RegConsoleCmd("rp_dances", Command_Menu);
	...
	**/
	...
	AutoExecConfig_SetFile("rp_emotes", "roleplay");
	...
	**/
	...
	g_EmoteForward = CreateGlobalForward("RP_OnEmote", ET_Ignore, Param_Cell);
	g_EmoteForward_Pre = CreateGlobalForward("RP_OnEmote_Pre", ET_Event, Param_Cell);
}
...
{
	RegPluginLibrary("rp_emotes");
	CreateNative("rp_IsClientEmoting", Native_IsClientEmoting);
	CreateNative("rp_StopEmote", Native_StopEmote);
...
int Native_StopEmote(Handle plugin, int numParams)
{
	StopEmote(GetNativeCell(1));
}
...
public Action OnPlayerRunCmd(int client, int &iButtons, int &iImpulse, float fVelocity[3], float fAngles[3], int &iWeapon)
{
	...
	if(!rp_GetClientBool(client, b_TouchedByDanceGrenade))
		StopEmote(client);
...
Action Command_Admin_Emotes(int client, int args)
{
	...
	LoopClients(i)
	{
		...
		{
			PerformEmote(client, target, amount);
		}	
	}	
